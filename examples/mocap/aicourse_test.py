...
def run_random_sequence(cf):
    ...
    # Start random trajectory
    while True:
        rand_x = random.uniform(-uni_bounds, uni_bounds)
        rand_y = random.uniform(-uni_bounds, uni_bounds)
        rand_z = random.uniform(-uni_bounds*0.7, uni_bounds*0.7)
        rand_yaw = random.uniform(-0.8 * uni_bounds, 0.8 * uni_bounds)
        rand_dur = random.uniform(0.3, 1)
        new_x = min(max(x + rand_x, -bound), bound)
        new_y = min(max(y + rand_y, -bound), bound)
        new_z = min(max(z + rand_z, 0.5), 2.5)
        new_yaw = min(max(yaw + rand_yaw, -1), 1)
        print(f"[{time.time() - t_start:.2f}s] Moving to ({new_x:.2f}, {new_y:.2f}, {new_z:.2f}, {new_yaw:.2f}) for {rand_dur:.2f} seconds")
        commander.go_to(new_x, new_y, new_z, new_yaw, rand_dur)
        x = new_x
        y = new_y
        z = new_z
        yaw = new_yaw
        time.sleep(rand_dur / 1.4)

        if batt_state == 3:
            print(f"[{time.time() - t_start:.2f}s] Battery low, landing")
            break

        # stop after 2 minutes
        if time.time() - t_start > 120:
            print(f"[{time.time() - t_start:.2f}s] Time limit reached, landing")
            break
...
def log_batt_callback(timestamp, data, logconf):
    global batt_level, batt_state
    print(f"[{time.time() - t_start:.2f}s] Battery level: {data["pm.vbat"]:0.2f}V, state: {data["pm.state"]}")
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        reset_estimator(cf)
        run_random_sequence(cf)
        time.sleep(1.0)
        stop_logconfig(log_config)
        # batt_level = get_battery_level(cf)
        # batt_state = get_battery_state(cf)
        # print(f"Battery level: {batt_level:.2f}V, state: {batt_state}")        
